...
module.exports = function (gulp, plugins, config) {
  ...
  return function () {
    ...
    var sassStream = gulp.src(dir(config.dirs.css + "/application.scss")) //add application.scss (where all the other scss files are included) to the event stream
      ...
      .pipe(plugins.sass({includePaths: [global.prefix + config.dirs.bower + "/"]})); //compile sass files into css (include paths is an array of directories where node-sass can look for files when @import-ing them)
